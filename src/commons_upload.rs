...
const MAX_PREPARATION_DIAGNOSTIC_BYTES: usize = 64 * 1024;
...
/// Provider media locator retained privately while a reviewed upload runs.
#[derive(Clone, Debug)]
pub enum CommonsAudioSource {
    /// Credential-free public page handled by yt-dlp.
    PublicPage(Url),
...
/// output remains inside that directory so it can be removed after upload.
///
/// # Errors
///
...
pub fn prepare_commons_opus(
    ...
    crate::private_files::create_private_directory(staging_directory)
        .map_err(|error| format!("Cannot create the private Commons staging directory: {error}"))?;
    match source {
        CommonsAudioSource::PublicPage(source_url) => {
            prepare_public_page_opus(source_url, staging_directory, yt_dlp_executable, progress)
        }
        CommonsAudioSource::YandexMusic { track_id, token } => {
            #[cfg(feature = "yandex-music")]
            {
...
fn prepare_public_page_opus(
    ...
    let stderr_thread = stderr.map(|mut stderr| {
        thread::spawn(move || {
            ...
            let _ = stderr
                .by_ref()
                .take(MAX_PREPARATION_DIAGNOSTIC_BYTES as u64)
...
fn validate_staged_opus_path(directory: &Path, path: PathBuf) -> Result<PathBuf, String> {
    let directory = fs::canonicalize(directory)
        .map_err(|error| format!("Cannot resolve the Commons staging directory: {error}"))?;
