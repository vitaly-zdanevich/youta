//! Regressions for retaining finished YouTube and Archive timelines without another load.

use super::*;

/// Builds a canonical finite Archive file or the existing YouTube test item.
fn fixture_end_pause_item(source: &SourceKind, title: &str) -> QueueItem {
    if *source == SourceKind::ArchiveOrg {
        let mut item = queue_item_from_direct(&DirectSourceInput {
            source: source.clone(),
            url: url::Url::parse("https://archive.org/download/fixture/chapter.opus")
                .expect("Archive audio URL"),
        });
        item.media.title = title.to_owned();
        item
    } else {
        fixture_youtube_item(title)
    }
}

#[test]
fn seeking_to_or_beyond_a_held_end_keeps_autoplay_release_available() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for command in [
            ...
            controller.play_queue_item(fixture_end_pause_item(&source, "Forward at end"), false);
...
#[test]
fn finite_youtube_and_archive_load_opt_into_native_end_pause_only() {
    for (source, kind, expected) in [
        (SourceKind::YouTube, MediaKind::Video, true),
        (SourceKind::YouTube, MediaKind::LiveStream, false),
        (SourceKind::ArchiveOrg, MediaKind::Audio, true),
        (SourceKind::ArchiveOrg, MediaKind::LiveStream, false),
        (SourceKind::RemoteFiles, MediaKind::Audio, false),
    ] {
        let (mut controller, state) = controller_with_mock_statuses([]);
        let mut item = fixture_end_pause_item(&source, "End pause fixture");
...
#[test]
fn held_finite_eof_preserves_timeline_progress_and_seek_context() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        let (mut controller, state, _, events) = controller_with_mock_lifecycle(
            [near_end_status()],
            [PlaybackEvent::MediaLoaded, PlaybackEvent::PlaybackStarted],
        ...
        controller.config.playback.autoplay = false;
        let item = fixture_end_pause_item(&source, "Retained media");
        ...
        assert_eq!(
            state.played.len(),
            1,
            "seeking must not reload or resolve the media again"
        );
        assert_eq!(
            state.commands,
            [
...
fn arrow_input_can_seek_when_held_eof_arrives_before_the_next_tick() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        ...
        controller.config.playback.autoplay = false;
        controller.play_queue_item(fixture_end_pause_item(&source, "Pending end"), false);
        controller.update_player();
        state.lock().unwrap().commands.clear();
        events
            .lock()
...
fn held_eof_releases_for_autoplay_repeat_or_an_explicit_queued_item() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for mode in ["autoplay", "repeat", "queue"] {
            ...
            controller.play_queue_item(fixture_end_pause_item(&source, "First"), false);
            if mode == "queue" {
    ...
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for repeat in [false, true] {
            ...
            controller.play_queue_item(
                fixture_end_pause_item(&source, "Enable continuation"),
                false,
            );
            controller.update_player();
            events
...
fn held_eof_before_playback_started_is_not_a_successful_pause() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        let (mut controller, state, _, events) = controller_with_mock_lifecycle([], []);
        controller.config.playback.autoplay = false;
        controller.play_queue_item(fixture_end_pause_item(&source, "Not decoded"), false);
        events
            .lock()
            .unwrap()
            .push_back(PlaybackEvent::EndOfFileHeld);
...
fn explicit_stop_and_error_still_clear_a_held_timeline() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for reason in [PlaybackEndReason::Stop, PlaybackEndReason::Error] {
            ...
            controller.config.playback.autoplay = false;
            controller.play_queue_item(
                fixture_end_pause_item(&source, "Stop retained media"),
                false,
            );
            controller.update_player();
            events
...
fn stale_status_cannot_unpause_or_move_a_retained_end_position() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for duration in [None, Some(Duration::from_secs(42))] {
            ...
            controller.config.playback.autoplay = false;
            controller.play_queue_item(fixture_end_pause_item(&source, "Snapshot race"), false);
            controller.update_player();
            events
                .lock()
                .unwrap()
...
fn seeking_back_from_held_eof_updates_the_view_to_playing_without_space() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for command in [
            ...
            controller.config.playback.autoplay = false;
            controller.play_queue_item(fixture_end_pause_item(&source, "Resume from end"), false);
            controller.update_player();
            events
                .lock()
                .unwrap()
...
fn seeking_while_manually_paused_keeps_the_view_paused() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        for command in [
            ...
            controller.config.playback.autoplay = false;
            controller.play_queue_item(fixture_end_pause_item(&source, "Manual pause"), false);
            controller.update_player();
            assert!(controller.view.playback.paused);
            state.lock().unwrap().commands.clear();
            assert!(controller.seek_player_command(command.clone()));
...
fn rewinding_and_finishing_again_does_not_reload_or_duplicate_history() {
    for source in [SourceKind::YouTube, SourceKind::ArchiveOrg] {
        ...
        controller.config.playback.autoplay = false;
        controller.play_queue_item(fixture_end_pause_item(&source, "Repeated end pause"), false);
        controller.update_player();
